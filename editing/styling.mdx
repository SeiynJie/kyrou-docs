...
- **`.touch-target`** (`src/index.css`) expands a control's hit area to roughly 48x48px via an
  `::after` pseudo-element, scoped to `@media (pointer: coarse)` only, so it changes nothing on
  desktop mouse/trackpad layout. Add it to small or icon-only touch controls (nav tabs, FAB,
  close buttons).
...
- **Sheet vs. dialog:** `Modal`, `QuickAddModal`, and `TaskDetailSheet` all render as a bottom
  sheet below `md` and a centered dialog at `md+`, sharing one class contract from
  `src/lib/sheetPresentation.ts` instead of each re-deriving the shape.
