...
| `src/components/ui/ContextMenu.tsx` | Shared right-click / long-press menu, driven by a config of menu items (built per-surface in `src/components/ui/contextMenuItems.tsx`); multi-select via right-click-drag comes from `src/features/shared/useRightClickMenu.ts` |
